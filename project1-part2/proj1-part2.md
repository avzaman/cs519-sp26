...
memory while performing parallel matrix multiplication. A critical requirement
in this project is rigorous performance profiling using the Linux perf tool to
identify, document, and resolve application-level performance overhead.
...
**Use the Linux perf tool** to collect CPU performance counter data.
Identify and categorize all sources of application-level overhead from perf output.
Apply targeted optimizations and verify improvement with follow-up perf runs.
...
After applying your ideas, re-run perf and include both the before and after
outputs in your report alongside a comparison table.
